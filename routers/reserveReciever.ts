...
reserveRecieverRouter.get("/profile", async (req, res) => {
  const { available_time, productId, userId, location_latitude, location_longtitude } = req.query as unknown as RecieverWaitingStatus_PageDto;
...
reserveRecieverRouter.get("/make-call", async (req, res) => {
  const { productId, userId } = req.query as unknown as RecieverWaitingStatus_PageDto;
